-- Seed Nayade's real email addresses into system_settings rows that are
-- currently empty. Uses UPDATE...WHERE value='' so it is a no-op on any
-- row that already has a value (idempotent, safe to re-run).
