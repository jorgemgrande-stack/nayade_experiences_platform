...
export default function PublicNav() {
  ...
  return (
    <header className={cn(
      ...
      transparent
        ? "bg-transparent"
        : "bg-white/96 backdrop-blur-md shadow-sm border-b border-border/60"
    )}>
      {/* Top info bar */}
      <div className={cn(
        "hidden lg:block border-b transition-all duration-300",
        transparent ? "border-white/15 bg-white/5" : "border-border/40 bg-primary/4"
      )}>
        <div className="container flex items-center justify-between py-1.5">
          <div className={cn(
            ...
            <span className="flex items-center gap-1.5">
              <Phone className="w-3 h-3" /> +34 919 041 947
            </span>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2.5 group cursor-pointer">
            <div className={cn(
              ...
              transparent ? "bg-white/20 backdrop-blur-sm" : "bg-primary"
            )}>
              ...
              >
                <Link href={link.href}>
                  <button className={cn(
                    ...
                    location === link.href && !transparent && "text-primary"
                  )}>
                    {link.label}
                    {link.children && (
          ...
          <div className="hidden lg:flex items-center gap-2">
            <Link href="/contacto">
              <Button variant="ghost" size="sm" className={cn(
                ...
                transparent ? "text-white/90 hover:text-white hover:bg-white/12" : "text-foreground hover:text-primary"
              )}>
                Contacto
              </Button>
            </Link>
            <Link href="/presupuesto">
              <Button size="sm" className="bg-accent hover:bg-accent/90 text-accent-foreground font-display font-semibold rounded-full px-5 shadow-sm">
        ...
        <div className="lg:hidden bg-white border-t border-border shadow-xl max-h-[80vh] overflow-y-auto">
          <div className="container py-4 space-y-1">
            ...
            <div className="pt-3 border-t border-border">
              <Link href="/presupuesto">
                <Button className="w-full bg-accent hover:bg-accent/90 text-accent-foreground font-display font-semibold rounded-full">
                  Solicitar Presupuesto
                </Button>
              </Link>
            </div>
