...
// ─── KPI Card — mismo estilo que CounterCard del CRM ─────────────────────────
const KPI_STYLES = {
...
function BookingBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    en_curso:   "bg-emerald-500/15 text-emerald-300 border-emerald-500/30",
...
function Panel({ title, icon: Icon, iconColor, children, action }: {
            ...
            <div className="flex items-center gap-2">
              {totalAlerts > 0 && (
                <div className="flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 rounded-lg px-3 py-1.5">
                  ...
                  <span className="text-rose-300 text-xs font-semibold">{totalAlerts} alerta{totalAlerts > 1 ? "s" : ""}</span>
                </div>
              )}
              <Link href="/admin/crm">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <div className="w-1 h-4 rounded-full bg-gradient-to-b from-emerald-400 to-blue-600" />
              <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-white/40">Resumen del mes</span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <KpiCard
                ...
                subLabel={`Total acumulado: ${fmt(kpis?.revenueTotal ?? 0)}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              {(alerts?.transfersToValidate ?? 0) > 0 && (
                <Link href="/admin/crm?tab=quotes">
                  <div className="flex items-center gap-3 bg-amber-500/8 border border-amber-500/25 rounded-xl px-4 py-3 cursor-pointer hover:bg-amber-500/12 transition-colors">
                    <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-amber-300">{alerts?.transfersToValidate} transferencia{alerts?.transfersToValidate! > 1 ? "s" : ""} sin validar</p>
                      <p className="text-[10px] text-amber-400/60">CRM → Presupuestos</p>
                    </div>
              ...
              {(alerts?.quotesExpiringSoon ?? 0) > 0 && (
                <Link href="/admin/crm?tab=quotes">
                  <div className="flex items-center gap-3 bg-orange-500/8 border border-orange-500/25 rounded-xl px-4 py-3 cursor-pointer hover:bg-orange-500/12 transition-colors">
                    <Clock className="w-4 h-4 text-orange-400 shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-orange-300">{alerts?.quotesExpiringSoon} presupuesto{alerts?.quotesExpiringSoon! > 1 ? "s" : ""} por vencer</p>
                      <p className="text-[10px] text-orange-400/60">Vencen en los próximos 7 días</p>
                    </div>
              ...
              {(alerts?.invoicesOverdue ?? 0) > 0 && (
                <Link href="/admin/crm?tab=invoices">
                  <div className="flex items-center gap-3 bg-rose-500/8 border border-rose-500/25 rounded-xl px-4 py-3 cursor-pointer hover:bg-rose-500/12 transition-colors">
                    <Banknote className="w-4 h-4 text-rose-400 shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-rose-300">{alerts?.invoicesOverdue} factura{alerts?.invoicesOverdue! > 1 ? "s" : ""} sin cobrar +30 días</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {(overview?.upcomingBookings?.length ?? 0) > 0 && (
                <Panel title="Próximas actividades (7 días)" icon={Clock} iconColor="text-white/40">
                  <div className="space-y-2">
              ...
              <Panel title="Acciones rápidas" icon={Zap} iconColor="text-amber-400">
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    { label: "Contabilidad",         desc: "Facturas y transacciones",        href: "/admin/contabilidad/facturas", icon: Banknote,     bg: "bg-rose-500/8 border-rose-500/20 hover:bg-rose-500/12",         ic: "text-rose-400" },
                    ...
                    {overview?.topExperiences?.map((exp, i) => (
                      <div key={exp.experienceId} className="flex items-center gap-3 bg-white/[0.03] rounded-lg px-3 py-2.5">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[10px] text-white/40">{exp.count} reserva{exp.count !== 1 ? "s" : ""} · {fmt(exp.revenue)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {(kpis?.quotesEnviados ?? 0) > 0 && (
                <Link href="/admin/crm?tab=quotes">
                  <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 hover:bg-blue-500/8 transition-colors cursor-pointer p-3.5 flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-blue-300">{kpis?.quotesEnviados} presupuesto{kpis?.quotesEnviados! > 1 ? "s" : ""} en espera de cliente</p>
