...
export default function ActivarProveedor() {
  ...
  const activate = trpc.suppliers.activateSupplierInvite.useMutation({
    onSuccess: (data) => { setUserName(data.name ?? ""); setDone(true); },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 max-w-sm w-full text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">Bienvenido, {userName}. Tu portal de proveedor está listo.</p>
          <Button className="w-full bg-orange-500 hover:bg-orange-600 text-white"
            onClick={() => navigate("/login?returnTo=/supplier/dashboard")}>
            Ir al portal
          </Button>
        </div>
      </div>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-sm w-full">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">Elige una contraseña para tu portal de proveedor</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative mt-1">
              <Input id="password" type={showPwd ? "text" : "password"} value={password}
                onChange={(e) => setPassword(e.target.value)} placeholder="Mínimo 6 caracteres" className="pr-10" required />
          ...
          <div>
            ...
            <Input id="confirm" type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)}
              placeholder="Repite la contraseña" className="mt-1" required />
          </div>
          ...
          <Button type="submit" className="w-full bg-orange-500 hover:bg-orange-600 text-white" disabled={activate.isPending}>
